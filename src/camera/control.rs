...
impl XimeaCamera {
    pub fn new(config: &CameraConfig) -> Result<Self> {
        ...
        let mut device = xiapi::open_device(Some(config.serial))
            .context("Failed to open XIMEA camera")?;
    ...
    fn set_framerate(&self, fps: f32) -> Result<()> {
        self.device.set_acq_timing_mode(xiapi::XI_ACQ_TIMING_MODE::XI_ACQ_TIMING_MODE_FRAME_RATE_LIMIT)
            .context("Failed to set acquisition timing mode")?;
        self.device.set_framerate(fps).context("Failed to set framerate")?;
        debug!("Framerate set to {} FPS", fps);
        Ok(())
    }
    ...
    fn optimize_network_performance(&self) -> Result<()> {
        ...
        debug!("Network performance parameters optimized");
        Ok(())
    }

    pub fn start_acquisition(&mut self) -> Result<xiapi::Acquisition> {
        info!("Starting image acquisition");
        self.device.start_acquisition().context("Failed to start acquisition")
    }

    pub fn stop_acquisition(&mut self) -> Result<()> {
        info!("Stopping image acquisition");
